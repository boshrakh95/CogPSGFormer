...
# Process and save ECG spectrograms from the STAGES dataset
# Segments the ECG signal into 2-minute windows (no overlap) and computes the spectrogram for each segment
# Chose 2-minute windows to match the raw ECG signal segmentation in "ECG_processing_raw_STAGES_v1.py"
...
for clinic in clinics:
    ...
    directory3 = "ecg_spectrogram_2min/"
    ...
    if len(dir_subjs) > 0:
        ...
        for subject in range(len(dir_subjs)):
            ...
            if raw:
                ...
                #  method2
                f, t, Zxx_ecg = compute_fft_spectrogram(eeg_signal=data_clean_segmented, fs=sf,
                                                        window_size=window_size, step_size=step_size, nfft=nfft)

                # matplotlib.use('TkAgg')
                # plt.figure(figsize=(10, 6))
                # plt.pcolormesh(t, f, Zxx_ecg[200, :, :], shading='gouraud')
                # plt.show()
