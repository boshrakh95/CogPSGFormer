...
# Update from v1: here only C3-M2 is selected for channel/feature consistency.
# C is better than F for spindle analysis that might be added in the future.
# referencing to mastoid is more frequent in the dataset. Also, referencing to earlobe retains more motion artifact
        ...
        if results:
            ...
            if len(filtered_data) > 0:
                ...
                for ind in range(len(bands)):
                    ...
                    relative.to_csv(os.path.join(path_sbj, bands[ind] + '_rel_power_all_eeg_channels_30sec.csv'),
                                    index=False)
                    absolute.to_csv(os.path.join(path_sbj, bands[ind] + '_abs_power_all_eeg_channels_30sec.csv'),
                                    index=False)
        else:
