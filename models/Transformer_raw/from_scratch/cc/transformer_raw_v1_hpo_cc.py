...
# fixed transformer arch, raw ECG/EEG signals
...
class MultiPathTransformerClassifier(nn.Module):
    def __init__(self, raw_dims, d_model_raw, nhead, num_layers_raw,
                 dim_feedforward_raw, dim_fc, output_dim,
        ...
        self.fc = nn.Linear((d_model_raw * 2), dim_fc)
    ...
    def forward(self, x_ecg, x_eeg):
        ...
        combined = torch.cat((x_ecg, x_eeg), dim=1)
        del x_ecg, x_eeg
...
class MyDataset(Dataset):
    ...
    def __init__(self, dir_raw, targets, valid_indexes, subj_indexes, subset, stats=None):
...
def train_model(model, criterion, optimizer, train_loader, val_loader, num_epochs, device,
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            x_ecg, x_eeg = map(lambda x: x.float(), [x_ecg, x_eeg])

            model = model.float()
            outputs = model(x_ecg, x_eeg)
...
def train_model_multiple_tasks(dir_raw, names_input, target_file, model_class, output_dir, device,
    ...
    for task in tasks:
        ...
        train_dataset = MyDataset(dir_raw_mod, y_task, valid_indices, train_subj, 'train')
        ...
        train_dataset = MyDataset(dir_raw_mod, y_task, valid_indices, train_subj, 'train', stats)
        # a = train_dataset[0]
        val_dataset = MyDataset(dir_raw_mod, y_task, valid_indices, val_subj, 'val', stats)
        test_dataset = MyDataset(dir_raw_mod, y_task, valid_indices, test_subj, 'test', stats)
        ...
        for conf_idx, conf in enumerate(selected_configs):
            ...
            model = model_class(raw_dims=raw_dims,
                                d_model_raw=conf_dict["d_model_raw"],
                                nhead=conf_dict["nhead"],
        ...
        best_model = model_class(
            raw_dims=raw_dims,
            d_model_raw=best_config["d_model_raw"],
...
hyperparams = {"d_model_raw": d_model_raw, "nhead": nhead,
