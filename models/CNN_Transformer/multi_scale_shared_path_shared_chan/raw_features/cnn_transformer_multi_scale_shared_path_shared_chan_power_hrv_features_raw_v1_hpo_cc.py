...
# proj head for raw data: 2 scales of 1D CNN, shared paths for scales, shared path for channels,
# concatenate at the end, projection head for features: MLP
...
class SharedMultiScaleCNNProjection(nn.Module):
    def __init__(self, d_model_raw, num_cnn_layers, kernel_sizes, stride=1, dropout=0.1):
        ...
        in_channels = 2  # Input has 2 channels (EEG + ECG)
        ...
        self.dropout = nn.Dropout(dropout)
        self.batch_norm = nn.BatchNorm1d(d_model_raw)
    ...
    def forward(self, x):
        ...
        for segment_idx in range(num_segments):
            segment = x[:, :, segment_idx, :]  # Shape -> (batch_size, 2, num_samples_per_segment)
...
class MultiPathTransformerClassifier(nn.Module):
    def __init__(self, feat_dims, raw_dims, d_model_feat, d_model_raw, nhead, num_layers_feat, num_layers_raw,
        ...
        # Linear projection layers for features
        ...
        # Shared Multi-Scale CNN projection head for combined raw data (EEG + ECG)
        self.project_raw = SharedMultiScaleCNNProjection(
        ...
        self.transformer_time_hrv = nn.ModuleList([
            TransformerBatchNormEncoderLayer(d_model_feat, nhead, dim_feedforward_feat, dropout, activation, norm)
            for _ in range(num_layers_feat)
        ])
        self.transformer_raw = nn.ModuleList([
            TransformerBatchNormEncoderLayer(d_model_raw * len(kernel_sizes), nhead, dim_feedforward_raw, dropout,
                                             activation, norm)
            for _ in range(num_layers_raw)
        ])

        # Fully connected layers
        self.fc = nn.Linear((d_model_feat * 3) + (d_model_raw * len(kernel_sizes)), dim_fc)
    ...
    def forward(self, x_time_hrv, x_freq_hrv, x_power, x_raw):
        ...
        x_raw = self.pos_enc2(x_raw.transpose(0, 1))

        # Pass through transformer encoders
        for layer in self.transformer_time_hrv:
            x_time_hrv = layer(x_time_hrv)
        for layer in self.transformer_raw:
            x_raw = layer(x_raw)

        # Take the last hidden state from each stream
        x_time_hrv = x_time_hrv[-1]
        x_freq_hrv = x_freq_hrv[-1]
        x_power = x_power[-1]
        x_raw = x_raw[-1]

        # Concatenate all streams and pass through FC layers
        combined = torch.cat((x_time_hrv, x_freq_hrv, x_power, x_raw), dim=1)
        combined = self.dropout(combined)
...
class MyDataset(Dataset):
    ...
    def resample_concat(eeg, ecg, target_sf):
        ...
        """
        num_segments_ecg, num_samples_ecg = ecg.shape  # (150, 10800)
        num_segments_eeg, num_samples_eeg = eeg.shape  # (600, 2100)

        # Step 1: Resample EEG from 70 Hz to 90 Hz
        resampled_eeg = resample(eeg, num=int(num_samples_eeg * (target_sf / 70)), axis=1)
        # Shape after resampling: (600, 2700)

        # Step 2: Resegment resampled EEG into 2-minute windows (matching ECG segments)
        # num_samples_per_segment = target_sf * seg_duration  # 90 * 120 = 10800
        resegmented_eeg = []
        for i in range(num_segments_ecg):
            resegmented_eeg.append(resampled_eeg[i * 4:(i + 1) * 4].reshape(-1))   # Combine 4 consecutive 30-sec seg
        resegmented_eeg = np.array(resegmented_eeg)
        # Shape after resegmentation: (150, 10800)
...
def train_model(model, criterion, optimizer, train_loader, val_loader, num_epochs, device,
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            x_raw = batch['raw'].to(device)
            labels = batch['label'].to(device)

            # Ensure all inputs are Float tensors
            x_power, x_time_hrv, x_freq_hrv, x_raw = map(lambda x: x.float(), [x_power, x_time_hrv, x_freq_hrv, x_raw])

            model = model.float()
            outputs = model(x_time_hrv, x_freq_hrv, x_power, x_raw)
...
def test_model(model, test_loader, device, task_output_dir, fold_dir, task,
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            x_power, x_time_hrv, x_freq_hrv, x_raw = map(lambda x: x.float(),
                                                                [x_power, x_time_hrv, x_freq_hrv, x_raw])
...
def train_model_multiple_tasks(dir_features, dir_raw, names_input, target_file, model_class, output_dir, device,
    ...
    tasks = ['pcet_concept_level_responses']
    ...
    for task in tasks:
        ...
        raw_dims = (train_dataset[0]['raw'].shape[-1])
