# update from test_rnn_power_hrv.py: modified for AHI prediction
...
class MultiPathLSTMClassifier(nn.Module):
    def __init__(self, input_dims, hidden_dim, num_layers, output_dim, dropout=0.1):
        """
        Args:
...
def create_output(dir_targets, subj_to_ret, path_save):
    ...
    df = pd.read_excel(dir_targets)
    missing_ids = [name for name in names_ret if name not in df['s_code'].values]
    ind_subj_to_remove = np.where(np.isin(names_ret, missing_ids))[0]
    ...
    ahi = df.loc[df['s_code'].isin(available_ids)].set_index('s_code').loc[available_ids, 'ahi'].values

    # Convert regression to binary classification
    # Find the class thresholds (median of the values)
    thresh_ahi = 5  # np.median(ahi)
    print(np.sum(ahi >= np.median(ahi)), "subj with high AHI")
    print(np.sum(ahi < np.median(ahi)), "subj with low AHI")
    # Convert
    ahi_class = (ahi >= thresh_ahi).astype(int)  # high: class 1, low: class 0
    # pvtb_rrt_new = pvtb_rrt_class[sub_id_retain]

    # np.save(path_save + "/ahi_values_classification_threshold_median_for_yasa_c3_eeg_rel_power_analysis.npy",
    #         ahi_class)  # (subj, 1) --> (936,)

    return ahi_class, ind_subj_to_remove
...
dir_targets = '/media/livia/Elements/public_sleep_data/stages/stages/original/De-identified Data/' \
...
y, ind_subj_to_remove = create_output(dir_targets, subj_retained_for_power_analysis, path_save)

# Retain only X for subjects with AHI values
X_power = np.delete(X_power, ind_subj_to_remove, axis=0)
X_hrv_t = np.delete(X_hrv_t, ind_subj_to_remove, axis=0)
X_hrv_f = np.delete(X_hrv_f, ind_subj_to_remove, axis=0)

# Read the saved organized targets
# y = np.load(path_save + "/ahi_values_classification_threshold_median_for_yasa_c3_eeg_rel_power_analysis.npy")
