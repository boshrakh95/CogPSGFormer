...
dir_targets = path_save + "/all_composite_scores_classification_for_yasa_c3_eeg_rel_power_analysis2.csv"
...
subj_retained_for_composite = pd.read_csv(path_file2 + "yasa_eeg_powers/subjects_retained_for_power_and_composite_all_analysis.csv", header=None)
subj_retained_for_composite = subj_retained_for_composite.values.flatten().tolist()[1:]  # Get the values and remove the header

# Read the data and retain only the subjects included in the composite analysis
X_power, X_hrv_t, X_hrv_f = read_input(path_save, names_ret, subj_retained_for_composite)

# Read the targets and only retain the desired composite score (column)
